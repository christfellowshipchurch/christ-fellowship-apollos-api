...
export default class Cache extends RedisCache.dataSource {
  ...
  KEY_TEMPLATES = {
    ...
    contentChannelItemIds: (_, contentChannelId) =>
      `content-channel-item-ids:${contentChannelId}`,
    contentItem: (_, id) => `content-item:${id}`,
    contentItemGuidId: (_, guid) => `content-item-id:${guid}`,
    contentItemChildren: (_, contentItemId) => `content-item:${contentItemId}:children`,
    definedType: (_, definedTypeId) => `defined-type:${definedTypeId}`,
    definedValue: (_, definedValueId) => `defined-value:${definedValueId}`,
    definedValueGuidId: (_, guid) => `defined-value-id:${guid}`,
    eventContentItems: `event-content-items`,
    group: (_, id) => `group:${id}`,
    groupExcludeIds: `group-exclude-ids`,
    groupLocations: (_, groupId) => `group:locations:${groupId}`,
    ...
    person: (_, personId) => `person:${personId}`,
