...
export default class Url extends RockApolloDataSource {
    ...
    getFromId(url) { // the url gets encoded as the id, so we can just return it with no fuss
        return url
    }
    ...
    async addToMasterList({ url, title }) {
         ...
         */
        if (status === 'inactive') {
            await this.patch(`/DefinedValues/${id}`, { IsActive: false });
            return url
        }
     ...
     */
    masterList = () => this.request('/DefinedValues')
        .filter(`DefinedTypeId eq ${UrlDefinedTypeId}`)
        .transform(results => results.map(({ value, description }) => 
            ({
                url: description,
                title: value
            })
