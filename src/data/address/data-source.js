...
export default class Address extends RockApolloDataSource {
  ...
  getByUser = async () => {
    ...
    if (family) {
      ...
      try {
        const location = await this.request(`/GroupLocations`)
          .filter(
            `
                        (GroupId eq ${familyId})
                        and
                        (GroupLocationTypeValueId eq ${get(
                          ApollosConfig,
                          'ROCK_MAPPINGS.LOCATION_TYPES.HOME_ADDRESS',
                          0
                        )})
                    `
          )
        ...
        console.log({ e });
        console.log(
          `This is likely because the the following family does not have an address associated with it: ${familyId}`
        );
      }
    }
    return null;
  ...
  updateByUser = async ({ street1, street2, city, state, postalCode }) => {
    const { Auth, Cache } = this.context.dataSources;
    ...
    if (location) {
      const { guid: locationGuid } = await this.request(`/Locations/${location}`).get();
      ...
      Cache.delete({ key: Cache.KEY_TEMPLATES.person`${personId}` });
